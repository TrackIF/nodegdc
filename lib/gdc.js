...
var gdc = function(email, password, project, logEnabled) {
  ...
  var GDC_REST_SERVER   = 'https://secure.gooddata.com/gdc';
  var GDC_FTP_SERVER    = 'secure-di.gooddata.com';
  ...
  var _request = function(type, uri, json, headerOverride) {
    ...
    request({ method: type, jar: true, headers: defaultHeaders, uri: uri, json: json || {} }, 
      ...
      switch(resp.statusCode) {
        ...
        case 400:
          d.reject('400: Missing Parameters');
          break;
...
/***************** Authentication Functions *****************/
  ...
  var login = function() {
    ...
    _getUserAuth()
      .then(function() { return _getUserToken(); })
      .then(function(data) {
        d.resolve(data);
      }, function(err) {
        if (err) d.reject(err);
...
/***************** Expose/Export Functionality *****************/
  ...
  this.GDC_FTP_SERVER = GDC_FTP_SERVER;
  this.GDC_API_ETL    = GDC_API_ETL;
  
  this.__request      = _request;
  this.__user         = {email: email, password: password}
